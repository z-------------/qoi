...
when isMainModule:
  ...
  while true:
    ...
    decoder.update(buf.toOpenArray(0, readSize - 1)) do (outData: seq[uint8]; bytesProcessed: int):
      debugEcho "processed ", bytesProcessed, " bytes into ", outData.len / decoder.channels.int, " pixels"
    if readSize < BufSize:
      break
